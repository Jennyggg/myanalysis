...
for fn in filessignal:
        ...
        arrays=tree.arrays(features_preselection1+features_preselection2+features_preselection3)
...
#backup: The uproot will turn subbranches into JaggedArrays, while numpy will automatically transfer JaggedArrays into ndarrays. This type transfer will not happen if all the JaggedArray is empty, which can cause ValueError. In this case we manually create an empty ndarray of the same dimension of the number of selected events instead. This error may still happen if the first file in the list contains some empty JaggedArray after preselection. In this case we just switch the order of the files in the list.
            else:
